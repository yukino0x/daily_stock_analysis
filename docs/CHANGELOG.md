...
- 🐛 **manual-ai-inputs JSON 序列化修复** — 导出包包含 `date`/`datetime`/`Enum` 等对象时不再抛出 `Object of type date is not JSON serializable`，改为统一转为可序列化字符串或枚举值。

### 新功能

- 🧾 **GitHub Actions 新增 `manual-ai-inputs` 模式** — 在 `daily_analysis.yml` 中支持只导出手动提问素材：实时股价 JSON、个股分析提示词、大盘复盘提示词、策略预设提示词（Agent skill instructions）；并写入 GitHub Actions outputs 与 `reports/` 文件，便于在工作流外手动向 AI 提问且尽量贴近 `stocks-only` 的前置输入。
