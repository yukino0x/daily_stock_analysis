...
class TelegramSender:
    ...
    def __init__(self, config: Config):
        ...
        Args:
            config: 配置对象
        """
        parse_mode = (getattr(config, 'telegram_parse_mode', 'Markdown') or 'Markdown').strip()
        if parse_mode not in {'Markdown', 'MarkdownV2', 'HTML'}:
            logger.warning("无效 TELEGRAM_PARSE_MODE=%s，回退为 Markdown", parse_mode)
            parse_mode = 'Markdown'
    ...
    def _send_telegram_message(self, api_url: str, chat_id: str, text: str, message_thread_id: Optional[str] = None) -> bool:
        ...
        configured_mode = self._telegram_config.get('parse_mode', 'Markdown')
        telegram_text = self._convert_text_for_parse_mode(text, configured_mode)

        payload = {
            "chat_id": chat_id,
            "text": telegram_text,
            "parse_mode": configured_mode,
        ...
        for attempt in range(1, max_retries + 1):
            ...
            if response.status_code == 200:
                ...
                # Parse error fallback chain: configured mode -> HTML -> plain text
                if 'parse' in error_desc.lower() or 'markdown' in error_desc.lower() or 'entity' in error_desc.lower():
                    if configured_mode != 'HTML':
                        logger.info("Telegram 解析失败，尝试 HTML 格式重发...")
                        html_payload = dict(payload)
                        html_payload['parse_mode'] = 'HTML'
                        html_payload['text'] = self._convert_to_telegram_html(text)
                        try:
                            html_resp = requests.post(api_url, json=html_payload, timeout=10)
                            if html_resp.status_code == 200 and html_resp.json().get('ok'):
                                logger.info("Telegram 消息发送成功（HTML 回退）")
                                return True
                        except (requests.exceptions.ConnectionError, requests.exceptions.Timeout) as e:
                            logger.error(f"Telegram HTML fallback failed: {e}")
                    ...
                    try:
                        plain_resp = requests.post(api_url, json=plain_payload, timeout=10)
    ...
    def _convert_text_for_parse_mode(self, text: str, parse_mode: str) -> str:
        """Convert Markdown-like content to a best-effort text for selected parse_mode."""
        if parse_mode == 'HTML':
            return self._convert_to_telegram_html(text)
        return self._convert_to_telegram_markdown(text)

    def _convert_to_telegram_html(self, text: str) -> str:
        """Best-effort Markdown to Telegram HTML conversion for fallback delivery."""
        result = html.escape(text)

        # Links: [text](url)
        result = re.sub(r'\[([^\]]+)\]\((https?://[^)\s]+)\)', r'<a href="\2">\1</a>', result)
        # Bold/italic/code
        result = re.sub(r'\*\*(.+?)\*\*', r'<b>\1</b>', result)
        result = re.sub(r'(?<!\*)\*(?!\*)(.+?)(?<!\*)\*(?!\*)', r'<i>\1</i>', result)
        result = re.sub(r'`([^`]+)`', r'<code>\1</code>', result)
        # Strip markdown headings symbol, keep text
        result = re.sub(r'(^|\n)#{1,6}\s+', r'\1', result)
